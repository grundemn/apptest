			  ...
			  openshift.withProject(env.NAMESPACE) {
			   withCredentials([
				...
				usernamePassword(credentialsId: "dev-reg-token", usernameVariable: "DEV_USER", passwordVariable: "DEV_PWD")
                  ...
                  sh "skopeo copy "${env.devreg}"/midtier/boot-isocode-xjxg066:latest docker://docker-registry-default.ospqa.gcom.grainger.com/midtier/boot-isocode-xjxg066:latest --src-creds \"$DEV_USER:$DEV_PWD\" --dest-creds \"$QA_USER:$QA_PWD\" --src-tls-verify=false --dest-tls-verify=false"
                    }
				}
